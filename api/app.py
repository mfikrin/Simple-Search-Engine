...
# from tf_idf import search_result
import csv
...
def fill_csv(og_table, tf_idf_table):
    ...
    # with open('api/original_table.csv','w+',newline="") as file_og:
    with open(os.path.join("/static/test", "original_table.csv"),'w+',newline="") as file_og:
        csv.writer(file_og).writerows(term_table_og)
    # with open('api/tfidf_table.csv','w+',newline="") as file_tf:
    with open(os.path.join("/static/test", "tfidf_table.csv"),'w+',newline="") as file_tf:
...
def search_result(query):   #data type of query is string
    ...
    for i in range(15):
        # data_file_og = open("api/test/data" + str(i + 1) + ".csv")
        data_file_og = open(os.path.join("/static/test/data" + str(i+1) + ".csv"))
        # data_file = open("api/test/data" + str(i+1) + ".txt")
        data_file = open(os.path.join("/static/test/data" + str(i+1) + ".txt"))
    ...
    for i in range(15):
        ...
        # dir_file = "api/test/data" + str(i+1) + ".csv"
        dir_file = os.path.join("/static/test/data" + str(i+1) + ".csv")
...
def search_engine():
    return render_template('search_engine.html', title='Search Engine')
...
def result():
    if request.method == 'GET':
        ...
        sr = search_result(query)

        message = None #will be changed if there is a document that match the query
        if sr[0]['sim'] == 0:
            message = "We found nothing that match your search query"
            sr = []
        return render_template('results.html', query_str = query, results = sr, error_message = message)

@app.route('/result_table/', methods = ['GET'])
def go_back():
    return redirect('/')

@app.route('/result_table/<query>', methods = ['GET'])
def result_table(query):
    if query == '':
        return redirect('/')
    else:
        #this is for ensuring when this link accessed not from the '/results' path
        sr = search_result(query) #fill in database for table
        # with open('api/original_table.csv', 'r') as tabel_data:
        with open(os.path.join("/static/test/original_table.csv"), 'r') as tabel_data:
            final_table = []
            
            tabel_csv = csv.DictReader(tabel_data)
            for row in tabel_csv:
                final_table.append(dict(row))
        
        return render_template('result_table.html', tabel = final_table, query_str = query)
